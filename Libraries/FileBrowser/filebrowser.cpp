...
#define info(asd)  Serial.print(F("Info: ")); Serial.println(F(asd))
...
FileBrowser::FileBrowser(const uint8_t sd_Pin){
	...
	// Initialize the SD card at SPI_FULL_SPEED. Use SPI_HALF_SPEED 
	//to avoid bus errors with breadboards.
...
bool FileBrowser::loadFileList() {
  ...
  //Serial.print("Inside loadFileList: ");
  //Serial.println(tmpFileCount);
...
bool FileBrowser::changeDir(char* FolderName) {
	...
	else if (String(FolderName).substring(0,2)=="./") {
...
bool FileBrowser::extractScene(uint8_t (&refScene)[MAX_LIGHT_STEPS][NUM_MUGS], uint8_t &refNumSteps, char (&refCurrentSetting)[STRING_LENGTH]) {
		...
		if(firstLine) {
			...
			for(int i=0; i<NUM_MUGS; i++) {
				 if (isdigit(stepBuf[i])) {
					refScene[p_numSteps][i] = stepBuf[i] - '0';//(uint8_t)strtoul(stepBuf[i], NULL, 16);
				 } else if(stepBuf[i] >= 'A' && stepBuf[i] <= 'F') {
					refScene[p_numSteps][i] = stepBuf[i] - '7';
				 }
				 else {
					refScene[p_numSteps][i] = 0; //Default is black
				 }
				 //Serial.print(refScene[p_numSteps][i]);Serial.print(',');
			 }
			 //Serial.println();
			p_numSteps++;
			//If the maximum number of files is reached stop
	...
	//Serial.print("NumSteps: "); Serial.println(p_numSteps);
