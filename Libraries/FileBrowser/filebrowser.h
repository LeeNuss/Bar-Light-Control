...
class FileBrowser {
  private:
	uint8_t SD_CHIP_SELECT;	// SD card SPI chip select pin.
	...
	SdFile FILE;// Use for file read/write in folders.
...
//----------------------------------------------
  public:
	...
	//Public variables
	char CURRENT_FILE_LIST[LIST_LENGTH][STRING_LENGTH];	//List of file names
	uint8_t CURRENT_INDEX_LIST[LIST_LENGTH];	//Index of files
