...
#define TLC_FADE_BUFFER_LENGTH    170
#endif
...
uint8_t tlc_updateFades(uint32_t currentMillisFade);
...
/** \addtogroup ExtendedFunctions
    \code #include "tlc_fades.h" \endcode
     ...
     - uint8_t tlc_updateFades(uint32_t currentMillisFade) - updates fades using
            currentMillisFade as the current time
...
/** Updates any running fades.
    \param currentMillisFade the current millis() time.
    \returns 0 if there are no fades left in the buffer. */
uint8_t tlc_updateFades(uint32_t currentMillisFade)
{	
    struct Tlc_Fade *end = tlc_fadeBuffer + tlc_fadeBufferSize;
    ...
    for (struct Tlc_Fade *p = tlc_fadeBuffer; p < end;){
        if (currentMillisFade >= p->endMillis) { // fade done
            ...
            if (currentMillisFade >= startMillis) {
                Tlc.set(p->channel, p->startValue + p->changeValue
                        * (int32_t)(currentMillisFade - startMillis)
